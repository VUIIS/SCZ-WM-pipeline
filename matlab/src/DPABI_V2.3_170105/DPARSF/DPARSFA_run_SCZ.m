...
% Gao: just add comments to original DPARSFA_run, no code modification 
function [Error]=DPARSFA_run_SCZ(AutoDataProcessParameter,WorkingDir,SubjectListFile,IsAllowGUI)
...
% Modified by YAN Chao-Gan 091001, Generate the pictures for checking
% normalization.s
...
%Convert T1 DICOM files to NIFTI images (NO)
...
%Reorient and Crop T1Img by using Chris Rorden's dcm2nii (NO)
% YAN Chao-Gan, 111121
...
%Apply Downloaded Reorient Mats      -- YAN Chao-Gan, 130615. (NO)
...
%Slice Timing (SCZ step 1 => FunImgA)
...
%Calculate the voxel-specific head motion translation in x, y, z and TDvox,FDvox (NO) 
%YAN Chao-Gan, 120819
...
%Reorient T1 Image Interactively (NO)
%Do not need parfor
...
%Reorient Functional Images Interactively (NO)
%Do not need parfor
...
%Coregister T1 Image to Functional space (SCZ: step 3 =>)
if (AutoDataProcessParameter.IsNeedT1CoregisterToFun==1)
    ...
    if ~exist('UseNoCoT1Image','var')
        ...
        if isempty(DirCo) % !!!!!!!!!!!!!!!!!!!!!!!!
            ...
            if length(DirImg)==1 %!!!!!!!!!!!!!!!!!!!!!!!!!!!! modified by GAO
%                 button = questdlg(['No co* T1 image (T1 image which is reoriented to the nearest orthogonal direction to ''canonical space'' and removed excess air surrounding the individual as well as parts of the neck below the cerebellum) is found. Do you want to use the T1 image without co? Such as: ',DirImg(1).name,'?'],'No co* T1 image is found','Yes','No','Yes');
%                 if strcmpi(button,'Yes')
                    UseNoCoT1Image=1;
%                 else
...
%Reorient Interactively After Coregistration for better orientation in Segmentation (NO)
%Do not need parfor
...
%Generating AutoMasks %140401 (NO)
...
% Segmentation (SCZ: step 4) (Cfg.IsSegment=2)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DARTEL and Normalize VBM results. %YAN Chao-Gan, 111111. (SCZ: step 5)
%Do Not Need Parfor
if (AutoDataProcessParameter.IsDARTEL==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        if i==1
            DirImg=dir('Template_6.*'); % 121x145x121
...
%%%%
%Normalize the T1 image in the case of unified segmentation (No New Segment nor DARTEL) (NO) 
%YAN Chao-Gan, 121217
if AutoDataProcessParameter.IsSegment==1
    for i=1:AutoDataProcessParameter.SubjectNum
...
%%%%%%%
%Genereated the masks based on Segmentation results (NO)
% YAN Chao-Gan, 140617
...
%%%%%%%%
% Warp the common-used masks into original space (NO)
% YAN Chao-Gan, 120822.
...
%Deal with the other covariables mask: warp into original space (NO)
...
%Remove the nuisance Covaribles (SCZ Step 5.5 => Remove noise, regressors = head motion regressors [Friston 24-parameter] + mean of CSF)
if (AutoDataProcessParameter.IsCovremove==1) && (strcmpi(AutoDataProcessParameter.Covremove.Timing,'AfterRealign'))
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        parfor i=1:AutoDataProcessParameter.SubjectNum
            ...
            %Polynomial trends
            %0: constant
            %1: constant + linear trend (default)
            %2: constant + linear trend + quadratic trend.
            %3: constant + linear trend + quadratic trend + cubic trend.   ...
            
            CovariablesDef.polort = AutoDataProcessParameter.Covremove.PolynomialTrend; % default 1

            %Head Motion Regressors (YES, Gao)
            ImgCovModel = 1; %Default
            CovariablesDef.CovMat=[]; % default Covremove.HeadMotion=4 (Friston 24-parameters)
            ...
            %Head Motion "Scrubbing" Regressors: each bad time point is a separate regressor (NO - Gao)
            if (AutoDataProcessParameter.Covremove.IsHeadMotionScrubbingRegressors==1)
                ...
                Index=find(FD > AutoDataProcessParameter.Covremove.HeadMotionScrubbingRegressors.FDThreshold); % bad points index (FD>0.5)
                TemporalMask(Index)=0; % bad points=0
                IndexPrevious=Index; % bad point index
                for iP=1:AutoDataProcessParameter.Covremove.HeadMotionScrubbingRegressors.PreviousPoints % previousPoints=1
                    IndexPrevious=IndexPrevious-1;
                    IndexPrevious=IndexPrevious(IndexPrevious>=1);
                    TemporalMask(IndexPrevious)=0;
                end
                IndexNext=Index; % bad point index
                for iN=1:AutoDataProcessParameter.Covremove.HeadMotionScrubbingRegressors.LaterPoints % laterPoints=2
                ...
                CovariablesDef.CovMat = [CovariablesDef.CovMat, BadTimePointsRegressor]; % Matrix of nuisance regressors (+ each bad point is a regressor) 
            end

            
            %Mask covariates CompCor methods %YAN Chao-Gan, 140628. Deal with different kind of nuisance covarites. (NO - Gao : CompCor method <Behzadi, 2007>)
            CompCorMasks = [];
            ...
            if ~isempty(CompCorMasks)
                ...
                CovariablesDef.CovMat = [CovariablesDef.CovMat, PCs]; % Matrix of nuisance regressors (+ ) 
            end
            
            
            %Mask covariates %YAN Chao-Gan, 140628. Deal with different kind of nuisance covarites. (YES -Gao : Mean of CSF) 
            ...
            if (AutoDataProcessParameter.Covremove.WholeBrain.IsRemove==1) % (NO - Gao: Whole Brain)
            ...
            % Add the other Covariate ROIs (NO - Gao)
            if ~isempty(AutoDataProcessParameter.Covremove.OtherCovariatesROI)
            ...
            %Extract Time course for the Mask covariates (YES - Gao)
            ...
            %Regressing out the covariates (YES - Gao)
...
%Filter (SCZ: step 6 => FunImgARF [0.01Hz, 0.1Hz], add mean back)
...
%Normalize on functional data (SCZ: step 7 => FunImgARFW, IsNormalize=3)
...
%Detrend (SCZ: step 8 => FunImgARFWD)
...
% YAN Chao-Gan, 140617
%Reslice WM and CSF masks to MNI functional space. (NO)
...
%Remove the nuisance Covaribles ('AfterNormalize') %YAN Chao-Gan, 140810.
%Change cov remove before filtering since DPARSFA 3.0. Previous is
%('AfterNormalizeFiltering') (NO)
...
%Calculate ALFF and fALFF  %YAN Chao-Gan, 120827 (ADNI: step)
...
%Calculate Degree Centrality (SCZ: step )
...
% Generate the appropriate ROI masks (NO)
% if (~isempty(AutoDataProcessParameter.CalFC.ROIDef)) || (AutoDataProcessParameter.IsDefineROIInteractively==1) % deleted by Gao
if (~AutoDataProcessParameter.IsCalFC) && (~isempty(AutoDataProcessParameter.CalFC.ROIDef)) || (AutoDataProcessParameter.IsDefineROIInteractively==1) % changed by Gao
...
%Functional Connectivity Calculation (by Seed based Correlation Anlyasis)(NO)
...
%Calculate CWAS: This should be performed in MNI Space (4*4*4) and only one session! (NO)
...
%%%%
%Normalize to symmetric template (NO)
...
%Smooth on functional data right before VMHC %YAN Chao-Gan, 151119 (NO)
...
%Calculate VMHC: This usually performed in MNI Space (NO)
