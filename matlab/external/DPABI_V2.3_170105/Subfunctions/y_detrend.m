...
for iCut=1:CUTNUMBER
    ...
    Volume_Poly=squeeze(AllVolume(:,Segment));
    poly_order=2;
    [poly_c,dummy]=multpolyfit(repmat((1:size(Volume_Poly,1))',1,size(Volume_Poly,2)),Volume_Poly,poly_order);
    poly_p=ones(1,size(Volume_Poly,1));
    for poly_i=1:poly_order
        poly_p=[poly_p;(1:size(Volume_Poly,1)).^poly_i];
    end

    img_poly_fit=poly_c'*poly_p;
    AllVolume(:,Segment)=Volume_Poly-img_poly_fit';
    fprintf('.');
end
